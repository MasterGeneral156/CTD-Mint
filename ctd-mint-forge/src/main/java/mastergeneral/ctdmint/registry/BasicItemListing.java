...
//Code from Neoforge, I'm sorry guys lol
//It works tho, so meh?!
public class BasicItemListing implements VillagerTrades.ItemListing {
    ...
    public @Nullable MerchantOffer getOffer(ServerLevel level, Entity p_219693_, RandomSource p_219694_) {
        ItemCost cost = new ItemCost(this.price.getItemHolder(), this.price.getCount(), DataComponentExactPredicate.EMPTY, this.price);
        Optional<ItemCost> optionalSecondCost = this.price2.isEmpty() ? Optional.empty() : Optional.of(new ItemCost(this.price2.getItemHolder(), this.price2.getCount(), DataComponentExactPredicate.EMPTY, this.price2));
